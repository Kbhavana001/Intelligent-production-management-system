...
import type { ProductData, KpiData } from '../types';
...
export const RcaModal: React.FC<RcaModalProps> = ({ isOpen, onClose, kpi, data }) => {
  ...
  useEffect(() => {
    if (isOpen && kpi) {
      const fetchAnalysis = async () => {
        ...
        setError(null);
        try {
          const result = await getRootCauseAnalysis(kpi.title, data);
          setAnalysis(result ?? "");
        } catch (err) {
      ...
      <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl border border-gray-700">
        <div className="p-6 border-b border-gray-700 flex justify-between items-center">
          <h2 className="text-xl font-semibold text-white">
            Root Cause Analysis for "{kpi.title}"
          </h2>
